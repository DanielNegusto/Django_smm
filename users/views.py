...
class RegisterView(FormView):
    ...
    success_url = reverse_lazy('mailing:home')  # Перенаправление на каталог
    ...
    def form_valid(self, form):
        ...
        message = (f'Здравствуйте, {user.username}! Пожалуйста, '
...
class CustomLoginView(LoginView):
    template_name = 'users/login.html'  # Укажите ваш шаблон для входа

    def form_valid(self, form):
        user = form.get_user()
        if not user.is_active:
            messages.error(self.request, 'Пожалуйста, проверьте свою почту и подтвердите регистрацию.')
            return self.form_invalid(form)  # Возвращаем неудачную форму, чтобы отобразить ошибки
        return super().form_valid(form)  # Вызов метода родителя для завершения входа

    def form_invalid(self, form):
        # Проверяем, есть ли пользователь с указанными данными
        email = form.cleaned_data.get('username')  # Или 'email', в зависимости от вашего AuthenticationForm
        if email:
            try:
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'Ваш аккаунт был активирован! Теперь вы можете войти.')
        return redirect('mailing:home')  # Перенаправьте на страницу входа или другую нужную страницу
    else:
