...
class AddMessageView(LoginRequiredMixin, UserMessagesAndRecipientsMixin, CreateView):
    model = Message
...
class DeleteObjectView(LoginRequiredMixin, UserMessagesAndRecipientsMixin, View):
    ...
    def get(request, pk, object_type):
        ...
        return render(request, 'mailing/delete_message.html', {
            'object': obj,
...
# CRUD for Recipients
class AddRecipientView(LoginRequiredMixin, UserMessagesAndRecipientsMixin, CreateView):
    model = Recipient
        ...
        for newsletter in context['newsletters']:
            if newsletter.start_time and newsletter.end_time:
                ...
                if remaining_time.total_seconds() <= 0:
                    ...
                    newsletter.remaining_hours = remaining_time.days * 24 + remaining_time.seconds // 3600
...
class StatisticsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['active_newsletters'] = user_newsletters.filter(status='Запущена').count()
        context['unique_recipients'] = Recipient.objects.filter(
            newsletter__in=user_newsletters).distinct().count()

        context['total_attempts'] = Attempt.objects.filter(
            newsletter__owner=user).count()
        context['successful_attempts'] = Attempt.objects.filter(newsletter__owner=user, status='Успешно').count()
        context['unsuccessful_attempts'] = Attempt.objects.filter(newsletter__owner=user, status='Не успешно').count()
